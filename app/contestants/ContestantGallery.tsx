  ...
  return <>
    ...
    <div className="contestant-profile-grid">{filtered.map((row) => <article className="contestant-profile-card" id={row.public_slug || row.id} key={row.id}>
      ...
      <div className="contestant-profile-copy">
        ...
        <p className="field-help">Each eligible $2.50 vote is a donation supporting the scholarship competition and mission. Final totals require verification and audit.</p>
      </div>
    </article>)}</div>
