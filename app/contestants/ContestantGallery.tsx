...
export function ContestantGallery() {
  ...
  return <>
    <div className="contestant-gallery-tools"><div><p className="eyebrow">THE NEW BEAUTY ISSUE · OFFICIAL CLASS</p><h2>Meet all {rows.length} contestants.</h2><p className="contestant-gallery-intro">Open any profile to discover her story, faith, platform, campaign video, and official voting link.</p></div><label className="field contestant-search"><span>Find a contestant or school</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by name, school, or platform…" /></label></div>
    <div className="contestant-directory">{filtered.map((row) => <details className="contestant-directory-row" id={row.public_slug || row.id} key={row.id}>
      <summary><span className="contestant-directory-photo">{row.headshot_public_path ? <img src={publicFileUrl(row.headshot_public_path)} alt={`${row.public_name || "Contestant"} official headshot`} /> : <span>PGWS</span>}<b>{row.contestant_number ? `#${String(row.contestant_number).padStart(3, "0")}` : "PGWS"}</b></span><span className="contestant-directory-identity"><small>MISS PGWS 2027 CONTESTANT</small><strong>{row.public_name}</strong><span>{row.college || "University not listed"}</span></span><span className="contestant-directory-platform"><small>HER PLATFORM</small><span>{row.platform || "Open her profile to learn about her platform and purpose."}</span></span><span className="contestant-directory-open">View full profile <b aria-hidden>↓</b></span></summary>
