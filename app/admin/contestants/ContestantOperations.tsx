...
type Contestant = { id: string; user_id: string; contestant_number: number | null; public_slug: string | null; public_name: string | null; college: string | null; headshot_public_path: string | null; campaign_video_url: string | null; instagram_url?: string | null; public_profile_status: string; updated_at: string };
...
export function ContestantOperations() {
  ...
  function exportVotingRoster() {
    const headings = ["Contestant number", "Contestant ID", "Public name", "Legal name", "Email", "School", "Headshot URL", "Profile status", "Video link", "Instagram link", "Performance points", "Verified votes"];
    const rows = contestants.map((item) => { const profile = profileByUser.get(item.user_id); return [item.contestant_number ? String(item.contestant_number).padStart(3, "0") : "", item.id, item.public_name, profile?.legal_name, profile?.email, item.college || profile?.college, item.headshot_public_path ? publicFileUrl(item.headshot_public_path) : "", item.public_profile_status, item.campaign_video_url, item.instagram_url, scoreByContestant.get(item.id) || 0, voteByContestant.get(item.id)?.verified_votes || 0]; });
