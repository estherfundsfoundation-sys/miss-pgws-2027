...
export function CampaignProfileEditor() {
  ...
  async function save(publish: boolean) {
    ...
    try {
      ...
      setProfile(saved.data?.[0] ?? profile);
      setHeadshot(null);
      setVideo(null);
      setMessage(publish
        ? "Your profile is live! Open the public contestant gallery to review it and share only your official link."
  ...
  return <div className="campaign-studio">
    ...
    <div className="campaign-studio-grid">
      <section className="panel campaign-progress-card">
        <div className="record-heading"><div><p className="eyebrow">PUBLICATION READINESS</p><h2>{completion}% ready</h2></div><span className={`status ${profile.public_profile_status === "published" ? "status--green" : ""}`}>{profile.public_profile_status.replace(/_/g, " ")}</span></div>
    ...
    <section className="panel campaign-editor-panel">
      ...
      <div className="form-grid">
        ...
        <label className="field field--wide"><span>Service and advocacy platform</span><textarea rows={6} value={platform} onChange={(event) => setPlatform(event.target.value)} placeholder="What issue will you advocate for, and what action do you want others to take?" required /><small>{platform.length} characters · minimum 40</small></label>
      ...
      <div className="campaign-publish-actions">
        ...
        <button className="button button--lipstick" disabled={busy || completion < 100} onClick={() => void save(true)}>{busy ? "Working…" : profile.public_profile_status === "published" ? "Update live profile" : "Publish profile"}</button>
