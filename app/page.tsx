...
export default function Home() {
  ...
  return <main>
    ...
    <section className="section section--blush"><div className="deadline-panel"><div className="deadline-copy"><p className="eyebrow">THE 2027 ISSUE · OPERATING IN 2026</p><h2>Your moment starts now.</h2><p className="lede">The public preview and interest community are open to eligible Christian college women. Review the official dates while final legal review is completed.</p><div className="deadline-list">{dates.map((item) => <div className="deadline-item" key={item.id}><b>{item.label}</b><span>{item.display}</span></div>)}</div><div className="hero-actions" style={{ marginTop: 28 }}><LaunchAction kind="applications"/><Link className="button button--paper" href="/timeline">Full timeline</Link></div></div><div className="deadline-art"><Image src="/brand/miss-pgws-2027-logo.png" alt="Official Miss Pretty Girls Who Serve 2027 logo" width={1536} height={1536} sizes="(max-width: 1040px) 100vw, 45vw" /></div></div></section>
