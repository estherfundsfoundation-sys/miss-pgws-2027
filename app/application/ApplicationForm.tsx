...
export function ApplicationForm() {
  ...
  async function submitApplication(){const session=getStoredSession();if(!session||!applicationId){setNotice('Please sign in before submitting your application.');return;}const profiles=await rest<{email_verified:boolean}[]>(`pgws_profiles?user_id=eq.${session.user.id}&select=email_verified&limit=1`);if(!profiles.data?.[0]?.email_verified){setNotice('Verify your email before final submission. Your draft will remain saved.');return;}const applications=await rest<{agreement_status:string}[]>(`pgws_applications?id=eq.${applicationId}&select=agreement_status&limit=1`);if(applications.data?.[0]?.agreement_status!=='signed'){setNotice('Read and sign the official agreement before submitting your application.');return;}const required=sections.flatMap(s=>s.questions).filter(q=>!['review-status','agreement'].includes(q.type)&&q.required!==false);const missing=required.filter(q=>!answers[q.key]);if(missing.length){setNotice(`Complete ${missing.length} remaining required response${missing.length===1?'':'s'} before submitting.`);return;}const result=await rest(`pgws_applications?id=eq.${applicationId}`,{method:'PATCH',body:JSON.stringify({answers,status:'submitted',completion_percent:100,submitted_at:new Date().toISOString(),locked_at:new Date().toISOString()})});setNotice(result.error||'Your application was submitted successfully. Staff can now review your verified record.');}
