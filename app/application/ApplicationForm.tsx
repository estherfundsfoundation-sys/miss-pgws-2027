...
function Field({ question, value, onChange, onFile, agreementStatus }: { question: Question; value: string | boolean | undefined; onChange: (value: string | boolean) => void; onFile?: (file:File)=>Promise<void>; agreementStatus: string }) {
  ...
  if (question.type === "agreement") {
    ...
    return <div className="notice field--wide"><span>{isSigned ? "✓" : "◆"}</span><div><strong>{isSigned ? "Agreement signed and attached" : question.label}</strong><br />{isSigned ? "Your signed agreement is verified in your application record. You do not need to sign it again." : "Open the clean 42-section agreement, initial the 15 required sections, then sign before final submission."}<div style={{marginTop:14}}>{isSigned ? <Link className="button button--ink button--small" href="/application">Continue application</Link> : <Link className="button button--ink button--small" href="/agreement">Open agreement</Link>}</div></div></div>;
  }
...
export function ApplicationForm() {
  ...
  const refreshApplication=useCallback(async(hydrateAnswers=false)=>{const session=getStoredSession();if(!session)return;const result=await rest<{id:string;answers:Answers;agreement_status:string}[]>(`pgws_applications?user_id=eq.${session.user.id}&select=id,answers,agreement_status&limit=1`);const application=result.data?.[0];if(application){setApplicationId(application.id);setAgreementStatus(application.agreement_status||'not_started');if(hydrateAnswers)setAnswers(application.answers||{});}},[]);
  useEffect(()=>{const draft=window.localStorage.getItem('miss-pgws-application-draft');if(draft){try{setAnswers(JSON.parse(draft));}catch{/* ignore invalid local draft */}}void refreshApplication(true);},[refreshApplication]);
  useEffect(()=>{const refresh=()=>void refreshApplication(false);const onVisibility=()=>{if(document.visibilityState==='visible')refresh();};window.addEventListener('focus',refresh);document.addEventListener('visibilitychange',onVisibility);return()=>{window.removeEventListener('focus',refresh);document.removeEventListener('visibilitychange',onVisibility);};},[refreshApplication]);
  ...
  async function submitApplication(){const session=getStoredSession();if(!session||!applicationId){setNotice('Please sign in before submitting your application.');return;}const profiles=await rest<{email_verified:boolean}[]>(`pgws_profiles?user_id=eq.${session.user.id}&select=email_verified&limit=1`);if(!profiles.data?.[0]?.email_verified){setNotice('Verify your email before final submission. Your draft will remain saved.');return;}const applications=await rest<{agreement_status:string}[]>(`pgws_applications?id=eq.${applicationId}&select=agreement_status&limit=1`);const currentAgreementStatus=applications.data?.[0]?.agreement_status||'not_started';setAgreementStatus(currentAgreementStatus);if(currentAgreementStatus!=='signed'){setNotice('Your agreement is not attached yet. Open the agreement, finish signing, and return here. This page will verify it automatically.');return;}const required=sections.flatMap(s=>s.questions).filter(q=>!['review-status','agreement'].includes(q.type)&&q.required===true);const missing=required.filter(q=>!answers[q.key]);if(missing.length){setNotice(`Complete ${missing.length} remaining required response${missing.length===1?'':'s'} before submitting.`);return;}const result=await rest(`pgws_applications?id=eq.${applicationId}`,{method:'PATCH',body:JSON.stringify({answers,status:'submitted',completion_percent:100,submitted_at:new Date().toISOString(),locked_at:new Date().toISOString()})});if(result.error)setNotice(result.error);else{setSubmitted(true);setNotice('Your application was submitted successfully. Staff can now review your verified record.');}}
  ...
  return <div className="form-layout">
    ...
    <section className="form-card">
      ...
      <form onSubmit={(event)=>{event.preventDefault(); if(step<sections.length-1)setStep(step+1);}}>
        <div className="form-grid">{visibleQuestions.map((question)=><Field key={question.key} question={question} value={answers[question.key]} onChange={(value)=>update(question.key,value)} onFile={(file)=>upload(question,file)} agreementStatus={agreementStatus} />)}</div>
