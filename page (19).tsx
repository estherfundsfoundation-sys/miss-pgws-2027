...
export function AgreementDocument() {
  ...
  function download(){const blob=new Blob([agreement.source.sourceMarkdown],{type:'text/markdown;charset=utf-8'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='Miss-PGWS-2027-Applicant-Contestant-Agreement.md';a.click();URL.revokeObjectURL(url);}
  async function signAgreement(){setSubmitMessage('');setSubmitting(true);try{const session=getStoredSession();if(!session)throw new Error('Please sign in before signing the agreement.');const versions=await rest<{id:string;version_label:string;source_sha256:string}[]>('pgws_agreement_versions?active=eq.true&governing_law_approved=eq.true&select=id,version_label,source_sha256&limit=1');const version=versions.data?.[0];if(!version)throw new Error('Staff must publish an attorney-approved agreement version before signatures can be accepted.');const applications=await rest<{id:string}[]>(`pgws_applications?user_id=eq.${session.user.id}&select=id&limit=1`);const applicationId=applications.data?.[0]?.id;if(!applicationId)throw new Error('Your application record is not ready. Sign out, sign back in, and try again.');if(!signatureData)throw new Error('Draw your signature before continuing.');const signatureBlob=await(await fetch(signatureData)).blob();const signatureFile=new File([signatureBlob],'electronic-signature.png',{type:'image/png'});const objectPath=`${session.user.id}/${applicationId}/agreement/${version.id}/signature.png`;const upload=await uploadPrivateFile(objectPath,signatureFile);if(upload.error)throw new Error(upload.error);const initialRows=requiredSections.map(section_number=>({application_id:applicationId,agreement_version_id:version.id,user_id:session.user.id,section_number,initials:initials[section_number].trim().toUpperCase()}));const initialsResult=await rest('pgws_agreement_initials',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify(initialRows)});if(initialsResult.error)throw new Error(initialsResult.error);const signedAt=new Date().toISOString();const applicantSnapshot={full_legal_name:fields['Full Legal Name'],email:fields['Applicant Email'],college:fields['College or University'],date_of_birth:fields['Date of Birth'],typed_legal_name:fields['Type Full Legal Name'],signed_at:signedAt,agreement_version:version.version_label,source_sha256:version.source_sha256};const snapshot=JSON.stringify({applicantSnapshot,acknowledgments:agreement.finalAcknowledgments,initials:initialRows.map(r=>({section:r.section_number,initials:r.initials}))});const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(snapshot));const hash=[...new Uint8Array(digest)].map(value=>value.toString(16).padStart(2,'0')).join('');const signatureResult=await rest('pgws_agreement_signatures',{method:'POST',body:JSON.stringify({application_id:applicationId,agreement_version_id:version.id,user_id:session.user.id,applicant_snapshot:applicantSnapshot,acknowledgment_snapshot:{items:agreement.finalAcknowledgments,accepted:true},signature_object_path:objectPath,signature_method:'drawn',final_checkbox:true,user_agent:navigator.userAgent,immutable_record_hash:hash})});if(signatureResult.error)throw new Error(signatureResult.error);await rest(`pgws_applications?id=eq.${applicationId}`,{method:'PATCH',body:JSON.stringify({agreement_status:'signed'})});setSubmitMessage('Your agreement was signed and attached to your application. Use Print / save PDF to retain your copy.');}catch(reason){setSubmitMessage(reason instanceof Error?reason.message:'The agreement could not be signed.');}finally{setSubmitting(false);}}
  ...
  return <div className="legal-layout">
    ...
    <article className="legal-document">
      ...
      <div className="legal-meta"><div><b>Agreement Version</b><br />Pending staff configuration</div><div><b>Effective Date</b><br />Pending staff configuration</div><div><b>Competition Year</b><br />{agreement.metadata.competitionYear}</div></div>
      <div className="notice no-print" style={{marginTop:18}}><span>◆</span><div><strong>Legal configuration required before launch.</strong><br />Section 41 intentionally preserves the attorney-approved governing-law placeholder. The system will not invent jurisdiction language.</div></div>
